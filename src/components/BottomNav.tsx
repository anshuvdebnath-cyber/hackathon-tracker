...
export const BottomNav: React.FC<BottomNavProps> = ({ currentView, setCurrentView }) => {
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 w-full z-40 bg-[#f9f9f9] border-t-[3px] border-[#1a1c1c] shadow-[0px_-4px_0px_0px_rgba(0,0,0,1)] flex items-stretch h-16 md:hidden"
    >
      {/* Home Tab */}
      <button
        id="tab-home-btn"
        onClick={() => setCurrentView('dashboard')}
        className={`flex flex-col items-center justify-center flex-1 font-mono text-xs font-bold uppercase transition-colors border-r-[2px] border-[#1a1c1c] ${
      ...
      <button
        ...
        className={`flex flex-col items-center justify-center flex-1 font-mono text-xs font-bold uppercase transition-colors ${
